...
- **Cancelled runs** are technical events, not statements: a generation the user aborted (an assistant message with stop reason `aborted`, often with empty or partial content) or a process cancelled mid-run records that the run stopped, never why. Record it as a Chronology context line — what was running, where it stopped — never as a ledger entry on its own. If the user meant something by cancelling, the meaning arrives in the next user message; attribute dissent there, where it is stated. A cancelled run is not dissent by reality either: nothing failed, and nothing contradicted a claim.
